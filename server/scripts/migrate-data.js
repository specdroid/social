...
// Minimal SQLite reader using better-sqlite3
let Database
try {
...
async function main() {
  ...
  if (!fs.existsSync(BACKUP_PATH)) {
    ...
    console.error('Run: cp prisma/dev.db prisma/dev.db.bak')
    process.exit(1)
  }
  ...
  const tableExists = (name) => !!backup.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(name)
  const safeAll = (table) => tableExists(table) ? backup.prepare(`SELECT * FROM "${table}"`).all() : []
